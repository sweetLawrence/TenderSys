...
// const { Tenders } = require('../Database/models');
...
//     if (!title || !description || !financial_year || !deadline || !document) {
//       return res.status(400).json({ message: 'All fields including file are required.' });
//     }
...
router.get('/', async (req, res) => {
  try {
    const tenders = await Tenders.findAll({
      attributes: ['id', 'title', 'description', 'financial_year', 'deadline', 'document']
    });
